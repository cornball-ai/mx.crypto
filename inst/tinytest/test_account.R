...
ids <- mxe_account_identity_keys(acct)
...
expect_equal(length(mxe_account_one_time_keys(acct)), 0L)

mxe_account_generate_one_time_keys(acct, 3L)
...
mxe_account_mark_published(acct)
expect_equal(length(mxe_account_one_time_keys(acct)), 0L)

# --- fallback key --------------------------------------------------------

fk <- mxe_account_fallback_key(acct)
...
blob <- mxe_account_pickle(acct, key)
expect_true(is.character(blob) && nchar(blob) > 0L)

acct2 <- mxe_account_unpickle(blob, key)
ids2 <- mxe_account_identity_keys(acct2)
expect_identical(ids$curve25519, ids2$curve25519)
expect_identical(ids$ed25519, ids2$ed25519)

# Wrong key fails
bad_key <- as.raw(rep(0xff, 32))
expect_error(mxe_account_unpickle(blob, bad_key))

# Wrong key length fails up front
expect_error(mxe_account_pickle(acct, raw(31)))
expect_error(mxe_account_pickle(acct, "not raw"))
