...
%\VignetteIndexEntry{Security audit (mx.crypto 0.1.0.1)}
-->
---
title: "Security audit (mx.crypto 0.1.0.1)"
---

# Security audit: mx.crypto 0.1.0.1

**Audit date:** 2026-05-13.
**Subject:** `mx.crypto` 0.1.0 (vodozemac 0.10.0 pin) → 0.1.0.1 with the fixes
described here.
...
Before 0.1.0.1 mx.crypto exposed 24 functions covering:
...
- `mxc_verify_one_time_key(key_object, signing_ed25519, expected_user_id, expected_device_id)` —
  same for a `/keys/claim` entry.

The two high-level helpers are pure R; they call `mxc_ed25519_verify`
(Rust) and `mx.api::mx_canonical_json` (pure R) for the signed-bytes
reconstruction.
...
when those endpoints landed in `mx.api` 0.1.0.1. Key properties of that
...
Before 0.1.0.1 the demo at `inst/integration/e2e_demo.R` (and any other
...
The full test suite is 25 assertions and lives in
`inst/tinytest/test_verify.R`.
...
| Check | Result |
|---|---|
| `tinytest::test_package("mx.crypto")` | 22 + 18 + 11 + 25 = 76 assertions, all pass |
...
- DESCRIPTION: bumped to 0.1.0.1; added `mx.api (>= 0.1.0.1)` and
